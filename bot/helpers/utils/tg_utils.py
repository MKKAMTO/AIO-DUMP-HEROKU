...
async def get_chats():
    ...
    for chat in database_chats:
        if chat is not None:
            if chat[0] not in allowed_chats and chat[0] is not None:
    ...
    for admin in database_admins:
        if admin is not None:
            if admin[0] not in admins and admin[0] is not None:
    ...
    if not Config.IS_BOT_PUBLIC == "True":
        ...
        if Config.AUTH_USERS == "":
            ...
            for user in database_users:
                if user is not None:
                    if user[0] not in allowed_users and user[0] is not None:
